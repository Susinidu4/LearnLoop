...
export const User_Register = () => {
  ...
  const handleChange = (e) => {
      ...
      if (name === 'name') {
        if (/[^a-zA-Z\s]/.test(value)) {
          return; // Don't update if contains numbers or symbols
        }
      }
  ...
  const checkPasswordStrength = (password) => {
    ...
    setPasswordStrength({
      score,
      message: messages.length > 0 ? `Requires: ${messages.join(', ')}` : '',
  ...
  return (
      <div className="flex w-full min-h-screen bg-[#f8f0e5] relative overflow-hidden">
        ...
        <div className="absolute top-0 right-72 w-[400px] h-[400px] rounded-full bg-[#4a2b0f] translate-x-1/2 -translate-y-1/2"></div>
        <div className="absolute -bottom-40 left-72 w-[400px] h-[400px] rounded-full bg-[#2F1B06] -translate-x-1/3 translate-y-1/3 z-20"></div>
        <div className="absolute -bottom-40 left-0 w-[600px] h-[600px] rounded-full bg-[#4a2b0f] -translate-x-1/3 translate-y-1/3 z-20"></div>
        ...
        <div className="w-full md:w-1/2 max-w-md">
                  ...
                  {formData.password && (
                    <div className="mt-2">
                      ...
                      {passwordStrength.message && (
                        <p className="mt-1 text-xs text-gray-600">{passwordStrength.message}</p>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="w-full md:w-1/2 flex flex-col items-center justify-center p-8">
            <img src={logo2} className="w-150 h-auto" alt="" />
            <img
              src={faqImg}
