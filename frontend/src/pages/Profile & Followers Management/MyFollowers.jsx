          ...
          const enrichedFollowers = await Promise.all(
            followersData.map(async (follower) => {
              try {
                ...
                console.error(
                  `Error loading data for user ${follower.followINGId}:`,
                  error
                );
                return {
                  ...follower,
            ...
            <div className="space-y-4">
              {followers.map((follower) => (
                <Link to={`/followerprofile/${follower.followINGId}`}>
                  <div
                    key={follower._id}
                    className="flex items-center mt-10 mx-40 bg-[#F0E0D1] rounded-lg shadow p-4 hover:shadow-md transition-shadow"
                  >
