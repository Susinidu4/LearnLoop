              ...
              <div className="bg-[#CFB397] shadow-md rounded-lg w-full max-w-4xl mx-auto mb-8 p-8">
                ...
                <div className="flex gap-8 mt-6">
                  <div className="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="red" viewBox="0 0 24 24" stroke="none" className="w-7 h-7">
                  ...
                  <div className="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-7 h-7">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 12l.084-.376a9.016 9.016 0 011.676-3.093A9.015 9.015 0 0112 3.75c4.478 0 8.214 3.29 8.91 7.583.066.4.09.808.09 1.217 0 4.28-3.53 7.75-7.89 7.75a8.09 8.09 0 01-2.939-.557c-.387-.144-.823-.083-1.146.158l-2.178 1.61a.75.75 0 01-1.18-.63v-2.614c0-.292-.115-.572-.318-.78a8.963 8.963 0 01-2.289-4.53L2.25 12z" />
                    </svg>
                ...
                <div className="mt-6">
                  <div className="h-[200px] overflow-y-scroll bg-[#8B6F5A] p-4 rounded-lg">
                    {comments.map((comment) => {
                      ...
                      return (
                        <div key={comment.id} className="flex justify-between items-center bg-[#D9C3AC] rounded-lg p-4 mb-6 min-h-[40px]">
                          <div className="flex-grow ml-4">
                            {editingCommentId === comment.id ? (
                              ...
                              <>
                                <p className="font-medium text-gray-800 break-words">{comment.content}</p>
                                <p className="text-xs text-gray-600 mt-1">
                                  on {new Date(comment.commentedAt).toLocaleString()}
                                </p>
                              </>
                            )}
                          </div>
                          ...
                          {editingCommentId !== comment.id && (
                            <div className="flex items-center gap-3">
                              <button
                                className={`text-[#8B6F5A] ${!isCommentOwner ? "opacity-50" : ""}`}
                                onClick={() => {
                                  if (isCommentOwner) {
                              ...
                              <button
                                className={`text-[#8B6F5A] ${!isCommentOwner && !isPostOwner ? "opacity-50" : ""}`}
                                onClick={() => {
