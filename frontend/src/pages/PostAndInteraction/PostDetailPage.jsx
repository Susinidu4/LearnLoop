...
export const PostDetailPage = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        const userDetails = await getUserById(fetchedPost.userId);
        setUser(userDetails);
    ...
    const fetchCommentAvatars = async () => {
      ...
      await Promise.all(
        comments.map(async (comment) => {
          if (!commenterAvatars[comment.userId]) {
            try {
              const imgUrl = await ProfileService.getProfileImage(
                comment.userId
              );
              avatars[comment.userId] = imgUrl;
            } catch (err) {
              avatars[comment.userId] = null; // fallback if image not found
            }
          }
        })
      );

      setCommenterAvatars((prev) => ({ ...prev, ...avatars }));
    };
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const userId = localStorage.getItem("userId"); // or however you're tracking the logged-in user
      ...
      if (response.ok) {
        const updatedPost = await response.json(); // the updated Post object returned from backend
        setPost(updatedPost);
                  ...
                  <div className="h-[200px] overflow-y-scroll bg-[#8B6F5A] p-4 rounded-lg">
                    {comments.map((comment, index) => (
                      ...
                      >
                        {/* Avatar */}
  <img
    src={commenterAvatars[comment.userId] || "/default-avatar.png"}
    alt="avatar"
    className="w-10 h-10 rounded-full object-cover"
  />

  {/* Comment Content with margin-left */}
